...
function concatFiles(fileList, outputFile) {
  ...
  return new Promise((resolve, reject) => {
    console.log(
      `ffmpeg -i "concat:${fileList.join("|")}" -acodec copy ${outputFile}`,
    );
    exec(
      `ffmpeg -i "concat:${fileList.join("|")}" -acodec copy ${outputFile}`,
      (error) => {
...
async function downloadAndConcat(objectKeys, outputFile) {
  const urls = objectKeys.map(
    (url) =>
      `https://r2.hktkdy.com/${encodeURIComponent(url).replaceAll("'", "%27")}`,
  );
...
export async function GET(req) {
  ...
  const result =
    await sql`select audio from words where due_date < ${endOfDayTimestamp} and audio is not null order by due_date`;
  ...
  try {
    ...
    return NextResponse.json(
      { message: "Success", outputFileName },
      { status: 200 },
    );
  } catch (error) {
