...
export async function GET(req) {
    try {
        ...
        const { tomorrowStartTimestamp, tomorrowEndTimestamp } = getTimestampForTomorrow();

        const result = await sql`select COUNT(*) from words where due_date < ${timestamp};`;
        const result2 = await sql`select COUNT(*) from words where due_date between ${tomorrowStartTimestamp} and ${tomorrowEndTimestamp};`;
        return NextResponse.json({ result:{now: result.rows[0].count, tomorrow: result2.rows[0].count}}, { status: 200 })
    }
    catch (error) {
